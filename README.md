...
`site` 항목: `name`, `tagline`, `url`(sitemap·robots에 쓴 주소와 맞추기), `feedbackForm`(구글 폼 주소, 비우면 "준비 중" 안내), `feedbackField`(폼에서 앱 이름이 들어갈 항목의 `entry.xxxxx` 값 — 폼의 "미리 채운 링크 받기"로 확인), `whispers`(속삭임에 추가할 문장 배열 — 비워도 됨).

**속삭임**(첫 화면에서 촛불을 움직이면 보이는 세 문장)은 `site.js`의 기본 84문장(미스터리 30 + 밤 감성 30 + 짧은 문장 24) + 서가 상태로 만드는 문장(봉인 수·비공개 수·작품 장 수·최근 앱) + `site.whispers`에서 올 때마다 3개를 무작위로 뽑는다. 직전 방문에 나온 문장은 피한다. 폰(≤560px)에서는 18자 이하 문장만 골라 한 줄로 두고, 그려 놓은 뒤 위치를 재서 층 밖(마스크에 잘리는 영역)으로 나가면 안쪽으로 밀어 넣는다(`fitWhispers`).
...
- 움직임은 **기본 켜짐**이고, OS의 「움직임 줄이기」(윈도우 "애니메이션 효과 끄기" 등)를 자동으로 따르지 않는다 — 그 설정이 켜진 PC에서 사이트가 통째로 밋밋해지던 문제(2026-09-08). 대신 헤더의 **✦ 효과** 버튼으로 끄고 켤 수 있고(localStorage `dgr-motion`, `html.reduce` 클래스), OS 설정이 감지되면 처음 한 번 안내 토스트를 띄운다.
- 공개 페이지(`body.site`)는 글자 선택·복사·길게 눌러 뜨는 메뉴를 막는다(`user-select:none`, copy/selectstart/Ctrl+A 차단) — 오터치에 글자가 잡히지 않게. 관리 화면은 해당 없음(2026-09-08).
- 히어로(`.hero`)는 `overflow-x:clip; overflow-y:visible`이다 — 세로까지 hidden이면 바닥의 안개 빛무리가 섹션 경계에서 잘려 히어로와 서가의 배경 톤이 갈라져 보인다(2026-09-08 수정). 서가(`.board`)는 `z-index:2`로 그 빛무리 위에 글자가 온다.
- 달은 스크롤하면 헤더 밑으로 들어가기 전에 스르르 사라진다(`--sy`로 opacity) — 헤더 블러에 번져 갑자기 커 보이던 현상 방지. 헤더의 블러·배경도 0.4초로 부드럽게 들어온다.
- 먼지 캔버스는 처음부터 가장 큰 화면 높이(`screen.height`)로 잡고 폭이 바뀔 때만 다시 만든다 — 폰에서 주소창이 접히고 펴질 때 점들이 순간이동하던 현상 방지.
- 카드 뷰어는 넘길 때 요소를 새로 만들지 않고 클래스만 승격(next→cur)해 CSS transition으로 미끄러진다 — 다음 장이 한 번 "자리잡는" 점프 방지.
